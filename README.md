...
```sh
xattr -cr /Applications/Obsidian-like.app
```
...
| App state for that vault | `<vault>/.obsidian-like/` — one JSON file per feature |
| Search index | `<vault>/.obsidian-like/index.db` — **a cache.** Delete it and it rebuilds from your files. |
